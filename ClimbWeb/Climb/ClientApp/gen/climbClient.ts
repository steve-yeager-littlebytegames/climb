...
export class GameApi extends BaseClass {
    ...
    addCharacters(request: AddCharacters): Promise<CharacterDto[]> {
        let url_ = this.baseUrl + "/api/v1/games/add-characters";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processAddCharacters(_response);
        });
    }

    protected processAddCharacters(response: Response): Promise<CharacterDto[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class SeasonApi extends BaseClass {
    ...
    extendOverdue(id: number | undefined, newDueDate: Date | undefined): Promise<SetDto[]> {
        let url_ = this.baseUrl + "/api/v1/seasons/extend-overdue?";
        if (id === null)
            throw new Error("The parameter 'id' cannot be null.");
        else if (id !== undefined)
            url_ += "id=" + encodeURIComponent("" + id) + "&"; 
        if (newDueDate === null)
            throw new Error("The parameter 'newDueDate' cannot be null.");
        else if (newDueDate !== undefined)
            url_ += "newDueDate=" + encodeURIComponent(newDueDate ? "" + newDueDate.toJSON() : "") + "&"; 
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processExtendOverdue(_response);
        });
    }

    protected processExtendOverdue(response: Response): Promise<SetDto[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class AddCharacters implements IAddCharacters {
    gameID!: number;
    names?: string[] | undefined;

    constructor(data?: IAddCharacters) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): AddCharacters {
        data = typeof data === 'object' ? data : {};
        let result = new AddCharacters();
        result.init(data);
        return result;
    }
...
export interface IAddCharacters {
    gameID: number;
    names?: string[] | undefined;
}
