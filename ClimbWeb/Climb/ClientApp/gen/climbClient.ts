...
export class AnalyzerApi extends BaseClass {
    ...
    analyze(player1ID: number | undefined, player2ID: number | undefined): Promise<AnalyzerData[]> {
    ...
    protected processAnalyze(response: Response): Promise<AnalyzerData[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            ...
            if (resultData200 && resultData200.constructor === Array) {
                ...
                for (let item of resultData200)
                    result200!.push(AnalyzerData.fromJS(item));
            }
            return result200;
            });
        } else if (status === 404) {
        ...
        return Promise.resolve<AnalyzerData[]>(<any>null);
    }
}
...
export class LeagueApi extends BaseClass {
    ...
    getSets(leagueUserID: number): Promise<FileResponse | null> {
        let url_ = this.baseUrl + "/api/v1/leagues/sets/{leagueUserID}";
        if (leagueUserID === undefined || leagueUserID === null)
            throw new Error("The parameter 'leagueUserID' must be defined.");
        url_ = url_.replace("{leagueUserID}", encodeURIComponent("" + leagueUserID)); 
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetSets(response: Response): Promise<FileResponse | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200 || status === 206) {
            const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
...
export class TournamentApi extends BaseClass {
    private http: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> };
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    get(tournamentID: number): Promise<TournamentDto> {
        let url_ = this.baseUrl + "/api/v1/tournaments/{tournamentID}";
    ...
    protected processGet(response: Response): Promise<TournamentDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? TournamentDto.fromJS(resultData200) : new TournamentDto();
            return result200;
            });
        } else if (status === 404) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<TournamentDto>(<any>null);
    }
    ...
    protected processGetAll(response: Response): Promise<TournamentDto[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    post(request: CreateRequest): Promise<TournamentDto> {
        let url_ = this.baseUrl + "/api/v1/tournaments";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processPost(response: Response): Promise<TournamentDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 201) {
            return response.text().then((_responseText) => {
            let result201: any = null;
            let resultData201 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result201 = resultData201 ? TournamentDto.fromJS(resultData201) : new TournamentDto();
            return result201;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
    ...
    generateBracket(id: number): Promise<TournamentDto> {
        let url_ = this.baseUrl + "/api/v1/tournaments/generate-bracket/{id}";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id)); 
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGenerateBracket(_response);
        });
    }

    protected processGenerateBracket(response: Response): Promise<TournamentDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class AnalyzerData implements IAnalyzerData {
    title?: string | undefined;
    player1Data?: string[] | undefined;
    player2Data?: string[] | undefined;

    constructor(data?: IAnalyzerData) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(data?: any) {
        if (data) {
            ...
            if (data["player1Data"] && data["player1Data"].constructor === Array) {
                this.player1Data = [] as any;
                for (let item of data["player1Data"])
                    this.player1Data!.push(item);
            }
            if (data["player2Data"] && data["player2Data"].constructor === Array) {
                this.player2Data = [] as any;
                for (let item of data["player2Data"])
                    this.player2Data!.push(item);
            }
        }
    }

    static fromJS(data: any): AnalyzerData {
        data = typeof data === 'object' ? data : {};
        let result = new AnalyzerData();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        ...
        if (this.player1Data && this.player1Data.constructor === Array) {
            data["player1Data"] = [];
            for (let item of this.player1Data)
                data["player1Data"].push(item);
        }
        if (this.player2Data && this.player2Data.constructor === Array) {
            data["player2Data"] = [];
            for (let item of this.player2Data)
                data["player2Data"].push(item);
        }
        return data; 
    }
}
