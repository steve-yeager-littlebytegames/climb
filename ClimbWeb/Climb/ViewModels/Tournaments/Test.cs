...
namespace Climb.ViewModels.Tournaments
{
    public class Test
    {
        ...
        public Test(BracketGenerator.Tournament tournament)
        {
            ...
            var winnersBracket = new int[tournament.Winners.Rounds.Count - 1][][];
            for(int i = 0; i < winnersBracket.Length; i++)
            {
                winnersBracket[i] = new int[tournament.Winners.Rounds[i].Games.Count][];
                for(int j = 0; j < tournament.Winners.Rounds[i].Games.Count; j++)
                {
            ...
            var losersBracket = new int[tournament.Losers.Rounds.Count][][];
            for(int i = 0; i < losersBracket.Length; i++)
            {
                losersBracket[i] = new int[tournament.Losers.Rounds[i].Games.Count][];
                for(int j = 0; j < tournament.Losers.Rounds[i].Games.Count; j++)
                {
